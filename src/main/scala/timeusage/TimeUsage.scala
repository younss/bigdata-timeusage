...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType =
    StructType(List(new StructField(columnNames.head, StringType, false)).union(
      columnNames.tail.map(new StructField(_, DoubleType, false))))
  ...
  def timeUsageSummary(
    ...
    val ageProjection: Column = when('teage.between(5, 22), "young")
